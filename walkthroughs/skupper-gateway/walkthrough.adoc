...
. Open the browser window pointing to the OpenShift Console of the Azure OpenShift cluster. Click on the openshift command line terminal on the top menu to open a terminal window.
+
...
. Initialize the Service Interconnect Router by issuing the below command in the azure terminal. This should install the Service Interconnect resources in the azure namespace
+
[source,bash,role=copy]
----
skupper init
