...
. Next setup the Corresponding connectors in the RHEL datacentre.
+
[source,bash,role=copy]
----
...
. Feel free toExamine the token using the below command
+
[source,bash,role=copy]
----
...
. Transfer the generate link configuration to the remote rhel datacentre.
+
[source,bash,role=copy]
----
...
. Open the browser window pointing to the OpenShift Console of the Azure OpenShift cluster link:{azure-console}/topology/ns/azure[Azure Developer Console, window="_blank"]. 
