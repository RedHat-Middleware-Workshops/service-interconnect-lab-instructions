...
. If you are logged out of the terminal for any reason. Click on the *Reconnect to terminal* button and issue the below commands to log back into the aws namespace
+
...
. If you are logged out of the terminal for any reason. Click on the *Reconnect to terminal* button and issue the below commands to log back into the azure namespace
+
image::images/reconnect_terminal.png[reconnect_terminal, role="integr8ly-img-responsive"]
+
[source,bash,role=copy]
...
. Go back to the patient portal or Click this link link:patient-portal-frontend-aws/{openshift-host}/topology/ns/aws[AWS Developer Console, window="_blank"] to access the patient portal.
...
This shows that the payment-processor in Azure cluster has taken over as soon as we made the processor in the datacentre unavailable over the network. As indicated in the image above the first payment was processed by RHEL datacentre and as soon as we took it down the the second payment was processed by Azure cluster.
...
=== Deleting the direct link between RHEL Datacentre and Azure cluster
...
. To verify if the connection, works properly lets try to make payment for another patient. Click this link link:patient-portal-frontend-aws/{openshift-host}/topology/ns/aws[AWS Developer Console, window="_blank"] to access the patient portal.
...
+
This indicates that though the direct link is broken the patient portal in AWS is able to reach the payment-processor in Azure
