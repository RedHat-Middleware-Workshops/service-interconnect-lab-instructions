...
. The terminal should take a couple of minutes to start up. Once done you should see something like this
+
...
The skupper cli is available through the RHEL datacentre terminal that you have connected earlier to using the terminal on your local machine.
...
[NOTE]
====
If you are logged out of the terminal for any reason. Click on the *Reconnect to terminal* button and issue the below commands
====
+
...
. Create proxy services that will redirect to the services running on the RHEL datacentre
+
[source,bash,role=copy]
----
...
. Now go to the browser tab where you've opened the patient-portal frontend or click this link link:patient-portal-frontend-aws/{openshift-host}/topology/ns/aws[AWS Developer Console, window="_blank"] to access it. 
...
Azure provides certified EU data centres that adhere to regulatory requirements.

=== Deployment overview: Azure cluster
. Open the console of the OpenShift cluster on Azure link:{azure-openshift-host}/topology/ns/azure[Azure Developer Console, window="_blank"]
...
. The terminal should take a couple of minutes to start up. Once done you should see something like this
+
image::images/azure_terminal_icon.png[azure_terminal_icon, role="integr8ly-img-responsive"]
...
. Navigate to the the Azure terminal
+
[NOTE]
====
If you are logged out of the terminal for any reason. Click on the *Reconnect to terminal* button and issue the below commands
...
. Create a new file on the Azure datacentre where you will paste the token you just generated on the AWS cluster.
+
[source,bash,role=copy]
----
vim secret_aws_azure.token
----
...
. Finally, you need to expose the payment-processor service over the link. This will allow the frontend app on the AWS cluster to connect to the payment-processor on Azure.
+
[source,bash,role=copy]
----
...
. Click this link link:patient-portal-frontend-aws/{openshift-host}/topology/ns/aws[AWS Developer Console, window="_blank"] to access the patient portal.

. Click on the Patient Dwight Schrute.
+
image::images/dwight.png[dwight, role="integr8ly-img-responsive"]

. Click the Bills tab to find the unpaid bills and hit the pay button.
+
image::images/bills_tab_dwight.png[bills_tab_dwight, role="integr8ly-img-responsive"]

. Submit the payment
+
image::images/submit_dwight.png[submit_dwight, role="integr8ly-img-responsive"]
...
+
image::images/dwight_datacentre.png[dwight_datacentre, role="integr8ly-img-responsive"]

. Now let's take down the payment-processor in the datacentre and see if the one in the Azure cluster takes over.  

. Navigate to the the RHEL datacentre terminal that you have connected earlier to using the terminal on your local machine. Make sure you are logged in as lab-user@bastion

. Instead of killing the container running the payment-processor in the datacentre, let's just unexpose it over the network and make it unaccessible. 
+
[source,bash,role=copy]
----
...
This shows that the payment-processor in Azure cluster has taken over as soon as we made the processor in the datacentre unavailable over the network.

[time=10]
[id="network-ha"]
== Scenario 3 - Enabling high availability of the service network connectivity with Red Hat Service Interconnect
